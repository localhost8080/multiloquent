...
- System font stack (`system`) — negative letter-spacing at display sizes for a tight, confident headline feel.
...
The theme registers three font families in `theme.json`. All headings, body copy, and UI default to `system` — the OS-native sans-serif stack:

- **`system` (default)** — `-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif`. Resolves to SF Pro on macOS/iOS, Segoe UI on Windows, and the native UI font on other platforms. No web font loading required.
- **`serif`** — `Georgia, 'Times New Roman', serif`. Available for editorial or long-form contexts where a traditional serif reading experience is appropriate.
- **`mono`** — `'Fira Code', 'Fira Mono', 'Cascadia Code', Consolas, monospace`. For code blocks and technical content.

### Registered Font Families (theme.json)

These are the three font families registered in `theme.json` → `settings.typography.fontFamilies` and available as block editor options:

| Slug     | Name              | Stack                                                                                    |
| -------- | ----------------- | ---------------------------------------------------------------------------------------- |
| `system` | System Sans-Serif | `-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif` |
| `serif`  | Serif             | `Georgia, 'Times New Roman', serif`                                                      |
| `mono`   | Monospace         | `'Fira Code', 'Fira Mono', 'Cascadia Code', Consolas, monospace`                        |
...
| `{typography.display-md}`     | 34px | 600    | 1.47        | -0.374px       | Section heads (system font at display proportions)     |
...
### Note on Font Rendering

The `system` font stack intentionally defers to the OS-native UI font on each platform — SF Pro on Apple, Segoe UI on Windows, and the native sans-serif elsewhere. No web font loading is required. Because each platform's system font has subtly different metrics, minor `letter-spacing` and `line-height` adjustments may be needed when cross-platform pixel-perfect consistency is critical.
...
**`button-primary`** — The signature Apple action. Background `{colors.primary}` (Action Blue #0066cc), text `{colors.on-primary}` in `{typography.body}` (system font 17px / 400), rounded `{rounded.pill}` (full pill — capsule-shaped), padding 11px × 22px. The full-pill radius IS the brand action signal.
...
5. Display headlines stay system font 600 with negative letter-spacing. Body stays system font 400 at 17px. The boundary is unbreakable.
