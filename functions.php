...
load_theme_textdomain(
    'multiloquent');
get_template_part(
    'classes/Bootstrap_Walker.class');
get_template_part(
    'classes/Shoestrap_Walker_Comment.class');
...
{
    register_nav_menus(
        array('header_menu' => __(
            'Header Navigation', 
            'multiloquent'),'footer_menu' => __(
            'Footer Navigation', 
            'multiloquent')));
}
...
if (function_exists(
    ...
    register_sidebars(
        (10), 
        array('before_widget' => '','after_widget' => '','before_title' => '<p class="nav-header">',
            'after_title' => '</p>','class' => ''));
}
add_filter(
...
function add_tag_class($taglinks)
{
    ...
    foreach ($tags as $tag) {
        $tagn[] = preg_replace(
            $regex, 
            "('$1$2 label tag-'.get_tag($2)->slug.'$3')", 
            $tag);
    }
    $taglinks = implode(
        '</a>', 
...
function breadcrumbs()
{
    ...
    if (is_tag()) {
        echo '<li><h5 style="margin:0;padding:0">Tag: ' . single_tag_title(
            '', 
            false) . '</h5></li>';
    }
    if (is_404()) {
    ...
    if (is_year()) {
        echo '<li><h5 style="margin:0;padding:0">' . get_the_time(
            'Y') . '</h5></li>';
    }
...
function jb_get_previous_posts_link($label = null)
{
    ...
    if (! is_single() && $paged > 1) {
        $attr = apply_filters(
            ...
            '');
        return '<a href="' . untrailingslashit(
            previous_posts(
...
function pagenavi($before = '', $after = '')
{
    ...
    if (! is_single()) {
        ...
        $pages_to_show = intval(
            $pagenavi_options['num_pages']);
        $larger_page_to_show = intval(
            $pagenavi_options['num_larger_page_numbers']);
        $larger_page_multiple = intval(
            $pagenavi_options['larger_page_numbers_multiple']);
        $pages_to_show_minus_1 = $pages_to_show - 1;
        $half_page_start = floor(
            $pages_to_show_minus_1 / 2);
        ...
        $larger_start_page_start = (round_num(
            $start_page, 
            10) + $larger_page_multiple) - $larger_per_page;
        $larger_start_page_end = round_num(
            $start_page, 
            10) + $larger_page_multiple;
        $larger_end_page_start = round_num(
            $end_page, 
            10) + $larger_page_multiple;
        $larger_end_page_end = round_num(
            $end_page, 
            10) + ($larger_per_page);
		...
		/*number_format_i18n(): Converts integer number to format based on locale (wp-includes/functions.php*/
            ...
            $prev_link = jb_get_previous_posts_link(
                $pagenavi_options['prev_text']);
            echo '<li>' . $prev_link . '</li>';
            if ($start_page >= 2 && $pages_to_show < $max_page) {
                $first_page_text = str_replace(
                    "%TOTAL_PAGES%", 
                    number_format_i18n(
                        $max_page), 
                ...
                echo '<li><a href="' . untrailingslashit(
                    esc_url(
                        get_pagenum_link())) . '" class="first" title="' . $first_page_text . '">1</a></li>';
                if (! empty(
                    $pagenavi_options['dotleft_text'])) {
            ...
            if ($larger_page_to_show > 0 && $larger_start_page_start > 0 && $larger_start_page_end <= $max_page) {
                for ($i = $larger_start_page_start; $i < $larger_start_page_end; $i += $larger_page_multiple) {
                    $page_text = str_replace(
                        ...
                        number_format_i18n(
                            $i), 
                        $pagenavi_options['page_text']);
                    echo '<li><a href="' . untrailingslashit(
                        esc_url(
                            get_pagenum_link(
                                $i))) . '" class="single_page" title="' . $page_text . '">' . $page_text . '</a></li>';
                }
            }
            for ($i = $start_page; $i <= $end_page; $i ++) {
                if ($i == $paged) {
                    $current_page_text = str_replace(
                        "%PAGE_NUMBER%", 
                        number_format_i18n(
                            $i), 
            ...
            if ($end_page < $max_page) {
                ...
                echo '<li><a href="' . untrailingslashit(
                    esc_url(
                        get_pagenum_link(
                            $max_page))) . '" class="last" title="' . $last_page_text . '">' . $max_page . '</a></li>';
            }
...
function jb_flex_slider()
{
    ...
    if (function_exists(
        ...
        $args = array('numberposts' => $total_posts,'offset' => 0,'category' => '','orderby' => 'post_date',
            'order' => 'DESC','include' => '','exclude' => '','post_type' => 'post','post_status' => 'publish');
    ...
    foreach ($recent_posts as $key => $val) {
        $slider_image = get_post_meta(
            $val->ID, 
            'bootstrap_slider', 
            true);
        $widthheight = ' width="1100" height="500" ';
        if ($slider_image) {
            $theimg = get_post_meta(
                $val->ID, 
                'bootstrap_slider', 
                true);
		...
		<p><a href="' . get_permalink(
            $val->ID) . '">';
        $output .= '<marquee scrollamount="6" behavior="alternate">' . trim(
            stripslashes(
                get_the_title(
                    $val->ID))) . '</marquee>';
...
{
    $input = array("swatch-red","swatch-orange","swatch-yellow","swatch-green","swatch-teal","swatch-blue",
        "swatch-violet","swatch-pink","swatch-mid-gray","swatch-gray");
    $apps = array("phone","appstore","calculator","compass","itunes","mail","music","weather","maps","videos","notes",
        "reminders","calendar","facebook","google","twitter","linkedin","finder","safari","firefox");
    if (! empty(
        $class) && in_array(
        $class, 
        $apps)) {
...
{
    $input = array("swatch-blue1","swatch-blue2","swatch-blue3","swatch-blue4","swatch-blue5","swatch-blue",
        "swatch-gray","swatch-violet");
    $apps = array("phone","appstore","calculator","compass","itunes","mail","music","weather","maps","videos","notes",
        "reminders","calendar","facebook","google","twitter","linkedin","finder","safari","firefox");
    if (! empty(
        $class) && in_array(
...
{
    $input = array("gradient-red","gradient-orange","gradient-yellow","gradient-green","gradient-teal","gradient-blue",
        "gradient-violet","gradient-magenta","gradient-black","gradient-silver");
    $apps = array("phone","appstore","calculator","compass","itunes","mail","music","weather","maps","videos","notes",
        "reminders","calendar","facebook","google","twitter","linkedin","finder","safari","firefox");
    if (! empty(
        $class) && in_array(
...
function jb_tiled_slider()
{
    ...
    foreach ($recent_posts as $key => $val) {
        ...
        if ($count == '6' || $count == '7' || $count == '9' || $count == '10') {
            $output .= '<h6 class="nowrap"><a href="' . get_permalink(
                $val->ID) . '">';
            $output .= '<span>' . trim(
                stripslashes(
                    get_the_title(
            ...
            $output .= '<h3 class="nowrap"><a href="' . get_permalink(
                $val->ID) . '">';
            $output .= '<span>' . trim(
                stripslashes(
                    get_the_title(
            ...
            if (strlen(
                ...
                $output .= '<h2 class="nowrap"><a href="' . get_permalink(
                    $val->ID) . '">' . trim(
                    stripslashes(
                        get_the_title(
                            $val->ID))) . '</a></h2>';
...
function get_PostViews($post_ID)
{
    ...
    if (is_admin()) {
        $resultscount = $wpdb->get_row(
            "select postnumber, cntaccess from $table_name WHERE postnumber = $post_ID");
        $cntaccess = number_format(
            (($resultscount) ? $resultscount->cntaccess : 0));
    }
    return $cntaccess;
}
...
{
    $tags = get_categories(
        'hide_empty=true&orderby=name&order=ASC&parent=' . $cat);
    ...
    if ($tags) {
        ...
        foreach ((array) $tags as $tag) {
            ...
            $new_tile_colour = get_random_solid_class(
                $tag->slug);
            ...
            $html .= "</li>";
            $html .= make_category_list_as_hierarchy(
                $tag->term_id);
            if ($cat == '0') {
                $html .= '</ul>';
            }
...
function jb_paralax_slider()
{
    ...
    foreach ($recent_posts as $key => $val) {
        ...
        $slider_image = wp_get_attachment_image_src(
        ...
        if ($count == '1') {
            ...
            $output .= '<img src="' . $theimg . '" class="grayscale" alt="' . trim(
                stripslashes(
                    get_the_title(
                        $val->ID))) . '" width="' . $width . '" height="' . $height . '">';
        ...
        $output .= '<div class="paralax_image_text"><h1><a href="' . get_permalink(
            $val->ID) . '">' . trim(
            stripslashes(
                get_the_title(
                    $val->ID))) . '</a></h1>';
...
function shoestrap_get_avatar($avatar)
{
    $avatar = str_replace(
        "class='avatar", 
        "class='avatar pull-left media-object", 
        $avatar);
    return $avatar;
}
add_filter(
    'get_avatar', 
    'shoestrap_get_avatar');
