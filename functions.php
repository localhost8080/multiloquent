...
function jb_get_previous_posts_link($label = null)
{
    ...
    if (! is_single() && $paged > 1) {
        ...
        return '<a href="' . untrailingslashit(previous_posts(false)) . "\" $attr>" . preg_replace(
            '/&([^#])(?![a-z]{1,8};)/i', 
            '&#038;$1', 
            $label) . '</a>';
    }
		...
		/*number_format_i18n(): Converts integer number to format based on locale (wp-includes/functions.php*/
            ...
            if ($larger_page_to_show > 0 && $larger_start_page_start > 0 && $larger_start_page_end <= $max_page) {
                for ($i = $larger_start_page_start; $i < $larger_start_page_end; $i += $larger_page_multiple) {
                    ...
                    echo '<li><a href="' .
                         untrailingslashit(esc_url(get_pagenum_link($i))) . '" class="single_page" title="' . $page_text .
                         '">' . $page_text . '</a></li>';
                }
            }
            ...
            if ($end_page < $max_page) {
                ...
                echo '<li><a href="' .
                     untrailingslashit(esc_url(get_pagenum_link($max_page))) . '" class="last" title="' . $last_page_text .
                     '">' . $max_page . '</a></li>';
            }
...
function jb_flex_slider()
{
    ...
    foreach ($recent_posts as $key => $val) {
        ...
        $output .= '"><img ' . $widthheight . ' src="' . $theimg . '" alt="image for ' . trim(
            stripslashes(get_the_title($val->ID))) . '"/>';
    ...
    foreach ($recent_posts as $key => $val) {
        ...
        if ($count == '6' || $count == '7' || $count == '9' || $count == '10') {
            ...
            if (strlen(trim(stripslashes(get_the_title($val->ID)))) > 40) {
                $output .= '<h3 class="nowrap"><a href="' . get_permalink($val->ID) . '">' . trim(
                    stripslashes(get_the_title($val->ID))) . '</a></h3>';
            } else {
                $output .= '<h2 class="nowrap"><a href="' . get_permalink($val->ID) . '">' . trim(
                    stripslashes(get_the_title($val->ID))) . '</a></h2>';
            }
        }
        $output .= '</li>';
        $count ++;
...
function jb_paralax_slider()
{
    ...
    foreach ($recent_posts as $key => $val) {
        ...
        $output .= '<div class="paralax_image_text"><h1><a href="' . get_permalink($val->ID) . '">' . trim(
            stripslashes(get_the_title($val->ID))) . '</a></h1>';
        $output .= '<p>';
