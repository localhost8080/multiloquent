...
{
    /*
     * Start of the <ul> Note on $depth: Counterintuitively, $depth here means the "depth right before we start this
     * menu". So basically add one to what you'd expect it to be
     */
    function start_lvl(&$output, $depth)
    {
        $tabs = str_repeat("\t", $depth);
        // If we are about to start the first submenu, we need to give it a dropdown-menu class
        if ($depth == 0 || $depth == 1) { // really, level-1 or level-2, because $depth is misleading here (see note
                                          // above)
    ...
    /*
     * End of the <ul> Note on $depth: Counterintuitively, $depth here means the "depth right before we start this
     * menu". So basically add one to what you'd expect it to be
     */
    ...
    /*
     * Add a 'hasChildren' property to the item Code from:
