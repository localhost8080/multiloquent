...
<body <?php body_class('bg-[var(--color-base)] text-[var(--color-contrast)] font-[var(--font-family-system)]'); ?>>
	...
	<!-- ===== Fixed top navbar ===== -->
	<?php $admin_bar_offset = is_admin_bar_showing() ? 'var(--wp-admin--admin-bar--height, 32px)' : '0'; ?>
	<header id="site-header"
		class="fixed inset-x-0 z-50 h-16 flex items-center px-4 gap-3
               bg-[var(--color-base)] border-b border-[var(--color-border)] shadow-sm"
		style="top:<?php echo $admin_bar_offset; ?>;"
		role="banner">
	...
	<!-- ===== Slide-in sidebar ===== -->
	<aside id="sidebar"
		class="fixed left-0 z-40 w-[var(--width-sidebar)]
              bg-[var(--color-surface)] border-r border-[var(--color-border)]
              overflow-y-auto -translate-x-full transition-transform duration-300 ease-in-out"
		style="top:calc(4rem + <?php echo $admin_bar_offset; ?>);height:calc(100dvh - 4rem - <?php echo $admin_bar_offset; ?>);"
